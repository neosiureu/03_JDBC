...
SELECT USER_NO, USER_ID, USER_PW, TB_USER.USER_NAME, TO_CHAR(ENROLL_DATE, ' YYYY "년" MM"월" DD "일" ' ) ENROLL_DATE  FROM TB_USER WHERE USER_NAME 
LIKE '%' || ? || '%'  ORDER BY USER_NO ;
...
SELECT * FROM TB_USER WHERE USER_ID = 'user07';

SELECT COUNT(*) FROM TB_USER WHERE USER_ID = 'user0111';


UPDATE TB_USER SET USER_NAME = '아이야'  WHERE USER_NO = 1;
COMMIT;